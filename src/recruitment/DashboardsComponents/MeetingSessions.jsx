...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleAddMeetingSession = async () => {
      // Add meeting session
        try {
          navigate('/CreateMeetingSession', { state: { id: id , page: currentPage } });
        } catch (error) {
    ...
    const handleEditMeetingSession = (meetingSessionId) => {
        try {
          navigate('/CreateMeetingSession', { state: { id: id, meetingSessionId: meetingSessionId, page: currentPage } });
        } catch (error) {
          console.error('Error adding meeting session:', error);
          alert('Error adding meeting session. Please try again later.');
        }
    ...
    const handleDeleteMeetingSession = async (meetingSessionId) => {
        try {
            if (meetingSessionId !== undefined && id) {
                const confirmed = confirm('Are you sure you want to delete this meeting session?');
                if (confirmed) {
                    await deleteMeetingSession(id, meetingSessionId);
                    const newMeetingSessions = meetingsSessions.filter((meetingSession) => meetingSession.id !== meetingSessionId);
        ...
        <table className="table-auto w-full border-collapse border border-gray-700 text-white rounded-lg text-sm">
            ...
            <tbody className="bg-white divide-y divide-gray-700">
                ...
                {PaginatedSessions.map((meetingSession, index) => (
                    <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-700' : 'bg-gray-800'} text-center` }>
                        ...
                        <td className="px-4 py-2 border border-gray-700">
                            <div className="max-h-[120px] overflow-y-auto ">
                                <button
                                    onClick={() => handleEditMeetingSession(meetingSession.id)}
                                ...
                                <button
                                    onClick={() => handleDeleteMeetingSession(meetingSession.id)}
