...
const Overview = ({ id, refresh, refresh2 }) => {
  ...
  return (
    <section className="relative h-screen-80 w-full p-2 bg-glass card ">
      <h1 className="text-3xl font-bold text-white mb-6">📊 Recruitment Overview</h1>
      <div className="h-screen-80 overflow-auto px-2">

{/* Current Stage */}
<div className="p-6 bg-gray-700 rounded-2xl shadow-lg text-white w-full overflow-x-auto mb-6">
  <h2 className="text-lg md:text-xl font-bold mb-4">Current Stage</h2>

  {/* Pasek postępu */}
  <div className="relative flex items-center justify-between min-w-[800px] px-10">
    {/* Pasek w tle */}
    <div className="absolute top-1/2 left-[6%] right-[6%] w-[87%] h-10 border-4 border-black rounded-full transform -translate-y-1/2"></div>

    {/* Pasek postępu */}
    <div
      className={`absolute top-1/2 left-[6%] h-8 transition-all duration-500 rounded-full ${
        stageColors[currentStageIndex]?.color || "bg-gray-500"
      }`}
      style={{
        width: `calc(${(currentStageIndex / (stageColors.length - 1)) * 87}% + 10px)`, // Pasek kończy się na kulce
    ...
    <div className="relative flex w-full justify-between">
      {stageColors.map((stage, index) => (
        <button
          ...
          className={`relative flex items-center justify-center w-20 h-20 md:w-24 md:h-24 rounded-full transition-all border-4 text-xs md:text-sm font-semibold text-center p-3 border-black ${
            index <= currentStageIndex
              ? `${stage.color} text-white` // Aktualny i wcześniejsze etapy
              : "bg-gray-600 text-gray-300" // Przyszłe etapy
          }`}
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 w-full">
          {/* Total Applicants */}
          <div className="p-4 md:p-6 bg-blue-600 rounded-2xl shadow-lg text-white flex items-center gap-4 w-full">
            <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M5.5 21a7 7 0 0 1 13 0M12 12a5 5 0 1 0-10 0 5 5 0 0 0 10 0ZM19 8v5M21.5 10.5h-5" />
            </svg>
            <div>
          ...
          {/* Highest Score */}
          <div className="p-4 md:p-6 bg-green-600 rounded-2xl shadow-lg text-white flex items-center gap-4 w-full">
            <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M3 3v18h18M7 12l3 3 7-7" />
            </svg>
            <div>
          ...
          {/* Average Score */}
          <div className="p-4 md:p-6 bg-yellow-500 rounded-2xl shadow-lg text-white flex items-center gap-4 w-full">
            <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M3 3v18h18M7 12l3 3 7-7" />
            </svg>
            <div>
          ...
          {/* Total Meetings */}
          <div className="p-4 md:p-6 bg-red-600 rounded-2xl shadow-lg text-white flex items-center gap-4 w-full">
            <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* Total Tasks */}
          <div className="p-4 md:p-6 bg-purple-600 rounded-2xl shadow-lg text-white flex items-center gap-4 w-full">
            <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M6 9l6 6 6-6" />
            </svg>
            <div>
        ...
        <div className="p-4 bg-slate-700 rounded-2xl shadow-lg text-white flex flex-col w-full mt-6">
            <h2 className="text-lg md:text-xl font-bold mb-4">Applicants in each stage</h2>
            <div className="flex flex-wrap gap-4 w-full">
              {stages.map(({ stage, color, iconPath }) => (
                <div
                  key={stage}
                  className={`p-4 rounded-xl shadow-lg text-white flex items-center gap-3 min-w-[120px] ${colorMap[color]}`}
                >
