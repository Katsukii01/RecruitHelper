...
const MeetingPoints = ({ id, refresh, onRefresh }) => {
  ...
  const [totalApplcants, setTotalApplcants] = useState(0);
  ...
  const fetchApplicants = async () => {
    try {
      ...
      setTotalApplcants(applicantsData.length);
    } catch (error) {
  ...
  const ChangeMeetingPoints = async (e,  meetingSessionId, meetingId) => {
    ...
    try {
      await updateMeetingPoints(id, meetingSessionId, meetingId, updatedValueNumber);
    ...
    try {
      ...
      const newMeetingSessions = meetingSessions.map(session => {
        if (session.id === meetingSessionId) {
          return {
            ...session,
            meetings: session.meetings.filter(meeting => meeting.id !== meetingId),
          };
        }
        return session;
      });
      ...
      alert("An error occurred while deleting your meeting. Please try again later.");
    }
  };
  ...
  const EditMeeting = (meetingSessionId, meetingId) => {
    ...
    if (MeetingSessionToEdit) {
      ...
      const MeetingToEdit = MeetingSessionToEdit.meetings.find(meeting => meeting.id === meetingId);
    ...
    <section className="relative w-full h-screen-80 mx-auto p-4 bg-glass card">
      ...
      <table className="table-auto w-full border-collapse border border-gray-700 text-white rounded-lg text-sm">
        <thead className="bg-gray-900 text-white">
          <tr>
            <th className="px-4 py-2 border border-gray-700 text-center">Applicant</th>
            {meetingSessions.map((session) => (
              <th key={session.id} className="px-4 py-2 border border-gray-700 text-center">
                <div>{session.meetingSessionName}</div>
                <div className="text-sm">Weight: {session.meetingSessionPointsWeight}</div>
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          {applicants.length > 0 ? (
            applicants.map((applicant, index) => (
              <tr key={applicant.id} className={`${index % 2 === 0 ? 'bg-gray-700' : 'bg-gray-800'} text-center`}>
                ...
                <td className="px-4 py-2 border border-gray-700">
                  <div className="max-h-[120px] overflow-y-auto">
                    ...
                    <div className="text-sm">Total Score: {applicant.totalScore}%</div>
                  </div>
                </td>

                {/* Kolumny dla spotkań */}
                {meetingSessions.map((session) => {
                  const applicantMeeting = session.meetings.find((m) => Number(m.applicantId) === Number(applicant.id));
                      ...
                      {applicantMeeting ? (
                        <>
                          <input
                            ...
                            onChange={(e) => ChangeMeetingPoints(e, session.id, applicantMeeting.id)}
            ...
            <tr>
              <td colSpan="100%" className="text-center text-white">No applicants available</td>
            </tr>
          )}
        </tbody>
      </table>
      ...
      {showConfirmDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center">
          <div className="bg-gray-800 p-6 rounded-md shadow-md w-96">
            ...
            <p className="text-white">Are you sure you want to delete this meeting?</p>
            <div className="flex justify-end gap-4 mt-4">
              ...
              <button
                onClick={() => DeleteMeeting(sessionIdToDelete, meetingIdToDelete)}
